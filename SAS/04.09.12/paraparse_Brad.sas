...
%MACRO PARAPARSE_Brad(INSET=, OUTSET=, FNAME_FULL=, TSTR=, NLINE=);
...
data _step1; 
	...
	END;
	/* Non Matches Output */
	if lastline and MATCH=0 then do;
        output;
    end;
	LINE=0; 
    MATCH=0; 
...
proc download data=_step1 out=_step1; run;



%put ## Step 2: Getting &NLINE. Lines Before and After Text;
%LINEPARAParse_Brad(INSET=_STEP1,OUTSET=&outset,FNAME_FULL=FNAME3,LINE=Line,NLINE=&NLINE)
