...
%macro split(npct=,colpre=,colyear=,inputds=,outputds=,col=); 
    ...
    *Determine number of observations in dataset with descriptions;
    ...
    /*%put ; %put ### Explode descriptions and delete temporary datasets;*/
    %do l = 1 %to &ndsn.; 
    ...
    /*%put ; %put ### Transpose descriptions and delete temporary datasets;*/
    %do m = 1 %to &ndsn.; 
        ...
        data &colpre.&m._out_t;
	        ...
	        &col. = compress(&col.,'_');
        run;
	    *Create ID and add year variable to *_out_t;
    ...
    /*%put ; %put ### Sort the datasets for merging and create summary statistics;*/
    %do m = 1 %to &ndsn.; 
        ...
        /*%put ; %put ### Create ID and add year variable to *_out_t_stats;*/
    ...
    /*%put ; %put ### Create global dictionary (year-level);*/
    data global_year;
        set %do m = 1 %to &ndsn.; &colpre.&m._out_t2a %end; ;  
    run;

	/*%put ; *%put ### Delete temporary datasets;*/
	...
	/*%put ; 
	%put #####################################################################;
    %put Global Dictionary (aggregate);
    %put #####################################################################;*/ 

	/*%put ; %put ### Create global dictionary (aggregate);*/
    data global(drop=&colyear.);
        set global_year;  
    run;

    /*%put ; %put ### Save global dictionary to test directories;*/
	...
	/*%put ; %put ### Compute summary statistics for global dictionary;*/
    ...
    /*%put ; %put ### Find the top x% global entries;*/
    proc sql noprint;
        select              count(&col.) 
	...
	/*%put ; %put ### Create global tables based on the percentiles;*/
    %do z = 1 %to &npct.;
        proc sql noprint;
    ...
    /*%put ; %put ### Sort the datasets for merging and create summary statistics for the global trimmed percentiles;*/
	%do z = 1 %to &npct.;
    ...
    /*%put ; %put ### Get the overall grand totals for global dataset;*/
    %do z = 1 %to &npct.;
        proc sql noprint;
	...
	/*%put ; %put ### Get the distinct grand totals for global dataset; */
	%do z = 1 %to &npct.;
    ...
    /*%put ; %put ### Combine global table *_stats_0 and *_stats_1_u2;*/
	%do z = 1 %to &npct.;
        proc sql noprint;
	...
	/*%put ; 
	%put #####################################################################;
    %put Global Dictionary (year-level);
    %put #####################################################################; */

    /*%put ; %put ### Compute summary statistics for global yearly dictionary;*/
	...
	/*%put ; %put ### Find the top x% global yearly entries;*/
	proc sql noprint;
    ...
    /*%put ; %put ### Create global tables based on the percentiles by year;*/
    %do z = 1 %to &npct.;
        proc sql noprint;
    ...
    /*%put ; %put ### Get the overall grand totals for global yearly dataset;*/
    %do z = 1 %to &npct.;
        proc sql noprint;
	...
	/*%put ; %put ### Get the distinct grand totals for global yearly dataset; */
	%do z = 1 %to &npct.;
	...
	/*%put ; 
	%put #####################################################################;
    %put Individual Dictionaries (aggregate);
    %put #####################################################################; */

	/*%put ; %put ### Create individual dicitonary for the most commonly used percentiles of words;*/
	%do z = 1 %to &npct.;
        proc sql noprint;
	...
	/*%put ; %put ### Add common word flags at aggregate-level;*/
    %do m = 1 %to &ndsn.;
    ...
    /*%put ; %put ### Create individual tables based on the percentiles;*/
    %do m = 1 %to &ndsn.;
        %do z = 1 %to &npct.;  
	        proc sql noprint;
	...
	/*%put ; %put ### Sort the datasets for merging and create summary statistics for the individual trimmed percentiles;*/
    %do m = 1 %to &ndsn.; 
	    %do z = 1 %to &npct.;  
	...
	/*%put ; %put ### Create a name variable for each table (should not have to do this for the final implementation);*/
    %do m = 1 %to &ndsn.; 
	 	data &colpre.&m._all_stats;
    ...
    /*%put ; %put ### Get the grand totals for each individual dataset;*/
    %do m = 1 %to &ndsn.; 
	    %do z = 1 %to &npct.;  
            proc sql noprint;
    ...
    /*%put ; %put ### Get the unique grand totals for each individual dataset;*/
    %do m = 1 %to &ndsn.; 
	    %do z = 1 %to &npct.;
	...
	/*%put ; %put ### Combine table *_stats_0 and *_stats_1_u2;*/
    %do m = 1 %to &ndsn.; 
	    %do z = 1 %to &npct.;
            proc sql noprint;
	...
	/*%put ; %put ### Rename final datasets to expand number formatting in title;*/
    %do m = 1 %to &ndsn.;
        proc sql noprint;
    ...
    /*%put Finish the agg-level;


	%put ; 
    %put #####################################################################;
    %put Individual Dictionaries (year-level);
    %put #####################################################################; 

	%put ; %put ### Create individual dicitonary for the most commonly used percentiles of words by year;*/
	%do z = 1 %to &npct.;
        proc sql noprint;
	...
	/*%put ; %put ### Add common word flags at year-level;*/
    %do m = 1 %to &ndsn.;
        proc sql noprint;
            select   a.&colyear.
            into     :temp_year
    ...
    /*%put ; %put ### Create individual tables based on the percentiles by year;*/
    ...
    /*%put ; %put ### Sort the datasets for merging and create summary statistics for the individual trimmed percentiles by year;*/
	...
	/*%put ; %put ### Create a name variable for each *_all_stats and stats_...PCT tables by year ;*/
    ...
    /*%put ; %put ### Get the grand totals by year for each individual dataset;*/
    ...
    /*%put ; %put ### Get the unique grand totals by year for each individual dataset;*/
    %do m = 1 %to &ndsn.; 
	    %do z = 1 %to &npct.;
	...
	/*%put ; %put ### Combine table *_stats_year_0 and *_stats_year_1_u2;*/
	...
	/*%put ; 
	%put #####################################################################;
    %put Merge _*_all_stats and _*_all_stats_year;
    %put #####################################################################; */
    ...
    %do m = 1 %to &ndsn.;
        ...
        quit;
        proc sql noprint;
            create table 		&temp_ds_name_comb._all_stats_combined as
            select  			a.id, a.&col., a._TYPE_, a._FREQ_, a.total_percentage, a.&colyear.
            ...
            and                 a.&col.=b.&col.;
        quit;
		proc sort data=&temp_ds_name_comb._all_stats_combined;
	...
	/*%put ; 
	%put #####################################################################;
    %put Create Global Summary tables (aggregate);
    %put #####################################################################; */

	/*%put ; %put ### Merge the percentile totals (aggregate);*/
	%do z = 1 %to &npct.;
	...
	/*%put ; %put ### Add the global counts so that the score can be created for the percentiles;*/
	%do z = 1 %to &npct.;
	    proc sql noprint;
    ...
    /*%put ; %put ### Create the scores and reorder the columns;*/
	%do z = 1 %to &npct.;
	...
	/*%put ; 
	%put #####################################################################;
    %put Create Global Summary tables (year-level);
    %put #####################################################################; */

	/*%put ; %put ### Merge the percentile totals (year-level);*/
	%do z = 1 %to &npct.;
	...
	%do z = 1 %to &npct.;
        ...
        *OLD;
	    proc sql noprint;
            select   glob_grnd_totals_year_&&Confidence_lbl_id&z., glob_dist_totals_year_&&Confidence_lbl_id&z. 
            into     :global_g_totals_year_&z., :global_d_totals_year_&z.
            from     global_stats_year_2_&&Confidence_lbl_id&z.;
        quit;
        data global_totals0_year_&&Confidence_lbl_id&z.;
	        set global_totals0_year_&&Confidence_lbl_id&z.;
		    global_grnd_tot_year_&&Confidence_lbl_id&z. = &&global_g_totals_year_&z.;
            global_dist_tot_year_&&Confidence_lbl_id&z. = &&global_d_totals_year_&z.;
	    run;
		*END OLD;
		*/
	...
	/*%put ; 
	%put #####################################################################;
    %put Combine Aggregate and Year-level Global Summary tables;
    %put #####################################################################; */
	...
	/*%put ; %put ### Create summary statistics;*/
	...
	/*%put ; %put ### Create a temp id variable so the tables can be joined;*/
    ...
    /*%put ; %put ### Merge the tables with the scores (Cannot do in loop);*/
    proc sql noprint;
    ...
    /*%put ; %put ### Merge the output from the macro with the dataset;*/
...
*%put ; *%put ### Call the macro Split that splits the descriptions into single datasets;
...
*%put ; 
*%put #####################################################################;
*%put Sign off if using SAS/Connect to WRDS;
*%put #####################################################################; 
